...
if ( isset( $_POST[ CT4GG_NAME . '-verif' ] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[ CT4GG_NAME . '-verif' ] ) ), CT4GG_NAME . '-opt' ) ) {
	...
	if ( isset( $_POST['ct4gg-humans'] ) ) {
		...
		if ( ! preg_match( '~humans*~', $humans_tmp ) ) {
			unset($_POST, $humans_tmp) ; $_POST = array();
		}
	}
	if ( isset( $_POST['submit-humans-restore'] ) ) {
		if ( $humans_file->backup() ) {
			if ( isset($humans_tmp ) ) {
				if ( copy( ABSPATH . $humans_tmp, ABSPATH . 'humans.txt' ) ) {
					self::view( 'humans', array( 'type' => 'copy-ok', 'nonce' => $humans_nonce ) );
				} else {
					self::view( 'humans', array( 'type' => 'copy-ko', 'nonce' => $humans_nonce ) );
				}
			} else {
				self::view( 'humans', array( 'type' => 'ct4gg-humans-ko', 'nonce' => $humans_nonce ) );
			}
		} else {
			self::view( 'humans', array( 'type' => 'backup-ko', 'nonce' => $humans_nonce ) );
		}
	} elseif ( isset( $_POST['submit-humans-delete'] ) ) {
		if ( isset( $humans_tmp ) ) {
			if ( unlink( ABSPATH . $humans_tmp ) ) {
				self::view( 'humans', array( 'type' => 'delete-ok', 'nonce' => $humans_nonce ) );
			} else {
				self::view( 'humans', array( 'type' => 'delete-ko', 'nonce' => $humans_nonce ) );
			}
		} else {
			self::view( 'humans', array( 'type' => 'ct4gg-humans-ko', 'nonce' => $humans_nonce ) );
		}
		...
		if ( file_exists( ABSPATH . 'humans.txt' ) ) {
			$humans_file->backup();
			$humans_file->save_mod( esc_txt( $_POST['humans-content'] ) );
		}
	} else {
		$humans_params = array(
			'humans_team',
		...
		if ( file_exists( ABSPATH . 'humans.txt' ) ) {
			if ( $humans_file->backup() ) {
				if ( ! $humans_file->save() ) {
					self::view( 'humans', array( 'type' => 'update-ko', 'nonce' => $humans_nonce ) );
				} else {
					self::view( 'humans', array( 'type' => 'update-ok', 'nonce' => $humans_nonce ) );
				}
			} else {
				self::view( 'humans', array( 'type' => 'backup-ko', 'nonce' => $humans_nonce ) );
			}
