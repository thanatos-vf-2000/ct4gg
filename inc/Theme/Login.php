...
class Login extends BaseController
{
	...
	{
		if (Options::get_option('login_screen_v2') == 1) {
			...
			if( !is_customize_preview() ) {
				...
				if ( ! empty( $options['ct4gg_field_register_link'] ) && $options['ct4gg_field_register_link'] === 1 ) {
					add_filter( 'register', array( $this, 'no_register_link' ) );
					add_filter( 'login_link_separator', array( $this, 'no_register_link' ) );
				}
				if ( ! empty( $options['ct4gg_field_lost_password'] ) && $options['ct4gg_field_lost_password'] === 1 ) {
					add_filter( 'login_link_separator', array( $this, 'no_register_link' ) );
				}
			}
		}
	 ...
	 */
	function ct4gg_sanitize_position( $input, $setting ) {
	...
	public function login_customize_register($wp_customize) {
		...
		foreach ($files as $file_key => $file_value)
		{
			if (!is_dir(CT4GG_PATH . 'inc/Theme/Login/Sections' . DIRECTORY_SEPARATOR . $file_value)) {
				unset($all_options);
				$all_options = Options::loadPHPConfig(CT4GG_PATH . 'inc/Theme/Login/Sections' . DIRECTORY_SEPARATOR . $file_value);
	...
	public function render_styles() {
		...
		*/
		...
		if ( ! empty( $options['ct4gg_logo_show'] ) && $options['ct4gg_logo_show'] === 1 ) {
		...
		*/
		...
		if ( ! empty( $options['ct4gg_bg_image_size'] ) ) {
			if ( $options['ct4gg_bg_image_size'] === 'custom' ) {
		...
		* Other Styling
		*/
		if ( ! empty( $options['ct4gg_field_back_blog'] ) && $options['ct4gg_field_back_blog'] === 1 ) {
